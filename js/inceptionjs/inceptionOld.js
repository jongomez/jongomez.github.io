function Inception(args) {  
  if(window.top === window.self) {
    // All iframes will be the child of the element with this Id.
    ...
    this.sameWidthAsUser = args.sameWidthAsUser || false;
    ...
    this.classesToRemove = this.classesToRemove === 'string' ? [this.classesToRemove] : this.classesToRemove;
...
Inception.prototype = {
  ...
  scaleIframe: function(iframe) {
    ...
    } else if(this.sameWidthAsUser) {
      // FIXME: Does this even work?
      elementToMatchWidth = iframe.scaleRatio ? window.top.innerHeight / iframe.scaleRatio : window.top.innerHeight;
      scaleRatio = containerWidth / elementToMatchWidth;
    } else {
      // DEFAULT - The iframe's scale ratio will be determined by the user's window.elementToMatch height.
      // i.e. the iframe will "see" the same height as the user sees.
      // XXX: When you're matching heights, what happens to the widths?
      elementToMatchHeight = window.top.innerHeight;
      scaleRatio = containerHeight / elementToMatchHeight;
    }

    //console.log("innerHeight:", window.top.innerHeight, "containerWidth:", containerHeight)
    //console.log("maxScrollTop:", iframe.maxScrollTop)
  ...
  mainDocumentLoadWait: function () {
    ...
    if(document.readyState != "complete") {
      ...
      window.top.addEventListener("load", this.firstTimeSetup.bind(this));
    } else {
  ...
  setupImageContainers: function(doc) {
    ...
    if(this.matrix3d != null) {
      ...
      //imageEl.style.width = this.imageWidth;
      //imageEl.style.height = this.imageHeight;
  ...
  firstTimeSetup: function() {
    ...
    iframeContainer = this.setupiframeContainer(window.top.document);
    if(iframeContainer != null) {
      this.addIframe(window.top.document, iframeContainer);
    }
  },
...
- Using contentDocument.documentElement.innerHTML = originalDoc.documentElement.innerHTML:
...
- Using contentDocument.write(originalDoc.documentElement.outerHTML) (with .open() and .close())
...
*/
  ...
  saveOriginalHTML: function() {
    ...
    for(var i = 0; i < this.classesToRemove.length; i++) {
      // this.originalDoc.documentElement || this.originalDoc
  ...
  addEventListeners: function() {
    ...
    if(this.trackResize) {
      window.top.addEventListener("resize", this.resizeHandler.bind(this))
    }

    // Scrolling.
    if(this.trackScroll) {
      // XXX: Event listener is added ONCE to the TOP WINDOW. All the other iframe windows don't have event listeners.
      window.top.addEventListener("scroll", this.handleScroll.bind(this));
    }
  },
  ...
  setupDocumentDimensions: function() {
    var body = window.top.document.body,
        html = window.top.document.documentElement;
  ...
  handleScroll: function() {
    ...
    maxYOffset = docDimensions[1] - window.top.innerHeight;
    scrollPercentage = window.top.pageYOffset / maxYOffset;
    
    //console.log("current doc height:", docDimensions[1])
    //console.log("offset:", window.top.pageYOffset, "Scroll %:", scrollPercentage);
  ...
  set3DScale: function(iframe) {
    ...
    doc = iframe == null ? window.top.document : iframe.contentDocument;
    ...
    // Why doesn't the wrap's height auto adjust?
    // To fix this, you need to set the wrap's height manually.
