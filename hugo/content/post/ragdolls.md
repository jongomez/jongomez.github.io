...
I tried playing with joints on the BabylonJS playground a little bit, but that didn't help. [They have a page explaining how to use these axes and points.](https://doc.babylonjs.com/how_to/joint_pivots), which helps a lot. But still, they have aproximately 10 examples in that page... and that's only for the HingeJoint. There are 8 joints in BabylonJS, I believe, and the same joint can behave differently according to the physics engine we're using. Knowing all this made me weary of using joints. Looking back though, there was no reason to be weary at all. In my opinion, the main things to remember are:
